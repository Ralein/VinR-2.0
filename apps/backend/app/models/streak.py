...
class Streak(Base):
    ...
    plan_id: Mapped[str] = mapped_column(
        String(255), ForeignKey("plans.id", ondelete="CASCADE"), nullable=False
    )
...
class DailyCompletion(Base):
    ...
    streak_id: Mapped[str] = mapped_column(
        String(255), ForeignKey("streaks.id", ondelete="CASCADE"), nullable=False, index=True
    )
    day_number: Mapped[int] = mapped_column(Integer, nullable=False)
    completed_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=datetime.utcnow
    )
    habit_completed: Mapped[bool | None] = mapped_column(Boolean, default=True)
