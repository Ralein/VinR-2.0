...
class Plan(Base):
    ...
    checkin_id: Mapped[str] = mapped_column(
        String(255), ForeignKey("checkins.id", ondelete="CASCADE"), unique=True, nullable=False
    )
    user_id: Mapped[str] = mapped_column(
        String(255), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True
    )
    immediate_relief: Mapped[dict] = mapped_column(JSON, nullable=False, default=list)
    daily_habits: Mapped[dict] = mapped_column(JSON, nullable=False, default=list)
